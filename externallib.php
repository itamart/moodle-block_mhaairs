...
class block_mhaairs_gradebookservice_external extends external_api {
    ...
    private static function get_course($courseid, $idtype = null) {
        ...
        // If the courseid is a positive integer, we search by course id.
        if ($ispositiveint) {
            $params = array('id' => (int) $courseid);
            $course = $DB->get_record('course', $params);
        }

        // If we didn't find the course yet and we are not restricted to id,
        // we can search by id number.
        if (!$course and !$idonly) {
